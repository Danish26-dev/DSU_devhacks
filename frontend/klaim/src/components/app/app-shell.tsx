...
const humanNav = [
  ...
  { to: "/app/verify", label: "Verification Requests", icon: ScanFace },
  { to: "/app/identity", label: "My Identity", icon: Fingerprint },
  { to: "/app/credentials", label: "Credentials", icon: WalletCards },
  { to: "/app/activity", label: "Verification Activity", icon: Activity },
...
function NavLinks({ role, onNavigate }: { role: Role; onNavigate?: () => void }) {
  ...
  const navItems: ReadonlyArray<{ to: string; label: string; icon: typeof LayoutDashboard }> =
...
function Brand({ role }: { role: Role }) {
  return (
    <Link to={role === "verifier" ? "/verifier/dashboard" : "/app/dashboard"} className="flex items-center gap-3 border-b border-border px-4 py-5">
...
export function AppShell({ role, children }: { role: Role; children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 flex-col justify-between border-r border-border bg-card lg:flex">
        <div>
          <Brand role={role} />
          <div className="p-3">
            <NavLinks role={role} />
      ...
      <header className="sticky top-0 z-40 flex items-center justify-between border-b border-border bg-background px-4 py-3 lg:hidden">
        <Link to={role === "verifier" ? "/verifier/dashboard" : "/app/dashboard"} className="flex items-center gap-2">
      ...
      {open ? (
        <div className="fixed inset-x-0 top-[57px] z-40 border-b border-border bg-background p-3 lg:hidden">
